...
public class createQuestionsPage {
	private final DatabaseHelper databaseHelper; //Initialize the databaseHelper
	
	public createQuestionsPage(DatabaseHelper databaseHelper){ //Constructor for setting database
	...
	public void show (Stage primaryStage, User user, QuestionList qList, AnswerList aList){ //Main Method
		...
		Label instructions = new Label("Create Question Below");
	    ...
	    post.setOnAction (a -> {
	    	...
	    	String postText = "";
	    	if (CharacterLengthEvaluator.evaluate(qTitle, "title") == true && CharacterLengthEvaluator.evaluate(qDescription, "description") == true) {
		    	qList.add(new Question(qTitle, qDescription, qUser));
		    	postText = "Question Posted";
	    	} else {
        ...
        layout.getChildren().addAll(instructions, title, description, post, back, postLabel); //Place all objects (Labels, Fields, Button) in Layout	
