...
public class deleteQuestionPage {
	...
	public deleteQuestionPage(DatabaseHelper databaseHelper){ //Constructor for setting database
	...
	public void show (Stage primaryStage, User user, QuestionList qList, AnswerList aList){ //Main Method
		...
		Label instructions = new Label("Delete Question Below (Type the Number)");
	    Label postLabel = new Label("");
	    TextField questionNumber = new TextField();
	    questionNumber.setPromptText("Enter Question Number to Delete");
	    ...
	    delete.setOnAction (a -> {
	    	...
	    	if (size > 0 && qNumber < size) {
			    if (qList.getList().get(qNumber) != null) {
			    	qList.delete(qNumber);
			    	postText = "Question Deleted";
			    } else {
			    	postText = "Error: Invalid Question Number";
			    }
	    	} else {
	    		postText = "Error: Invalid Question Number";
	    	}
        ...
        layout.getChildren().addAll(instructions,questionNumber,delete, back, postLabel); //Place all objects (Labels, Fields, Button) in Layout	
